...
flags.DEFINE_string("input_dir", "/data2/paarth/defenseData/Original_wav/", "location of Librispeech")
flags.DEFINE_string("output_dir", "/data2/paarth/defenseData/Adversarial_Yao_Robust/", "Output Dir")

flags.DEFINE_string("output_dir_speechrir_stage1", "/data2/paarth/defenseData/Adversarial_Yao_Robust_speechrir_stage1/", "Output Dir speechrir_stage1")
flags.DEFINE_string("output_dir_perturb_stage1", "/data2/paarth/defenseData/Adversarial_Yao_Robust_perturb_stage1/", "Output Dir perturb_stage1")
flags.DEFINE_string("output_dir_perturb_stage2", "/data2/paarth/defenseData/Adversarial_Yao_Robust_perturb_stage2/", "Output Dir perturb_stage2")
...
# data processing
flags.DEFINE_integer('max_length_dataset', '123265', 'the length of the longest audio in the whole dataset')
...
def ReadFromWav(data, batch_size):
    ...
    for i in range(batch_size):
        sample_rate_np, audio_temp = wav.read(FLAGS.input_dir + str(data[0, i]))
...
def main(argv):
    ...
    if not os.path.isdir(FLAGS.output_dir_speechrir_stage1):
        os.makedirs(FLAGS.output_dir_speechrir_stage1)

    if not os.path.isdir(FLAGS.output_dir_perturb_stage1):
        os.makedirs(FLAGS.output_dir_perturb_stage1)

    if not os.path.isdir(FLAGS.output_dir_perturb_stage2):
        os.makedirs(FLAGS.output_dir_perturb_stage2)
        ...
        with tf.Session(config=tfconf) as sess: 
            ...
            for l in range(num_loops):
                ...
                for i in range(batch_size):
                    ...
                    name, _ = data_sub[0, i].split(".")                    
                    saved_name = FLAGS.output_dir + str(name) + ".wav"                     
                    adv_example_float =  adv_example[i] / 32768.
                    wav.write(saved_name, 16000, np.array(np.clip(adv_example_float[:lengths[i]], -2**15, 2**15-1)))

                    saved_name = FLAGS.output_dir_perturb_stage2 + str(name) + ".wav"                     
                    perturb_float =  perturb[i] / 32768.
                    wav.write(saved_name, 16000, np.array(np.clip(perturb_float[:lengths[i]], -2**15, 2**15-1)))
                    print(saved_name)
